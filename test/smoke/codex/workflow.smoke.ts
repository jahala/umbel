...
smokeDescribeFor('codex', 'codex workflow two-step picker/transformer', () => {
  const guard = makeCleanupGuard();

  afterEach(() => guard.cleanup());

  test('codex two-step workflow completes and transformer echoes picker color', async () => {
    // Validates: workflow YAML executor with codex provider, step output capture via
    // assistant_last_message, {{var}} substitution, status.json written as 'completed'
    const tmpDir = await mkdtemp(join(tmpdir(), 'umbel-smoke-cdx-wf-'));

    const yamlContent = `
workers:
  picker: { provider: codex, cwd: "${tmpDir}" }
  transformer: { provider: codex, cwd: "${tmpDir}" }
steps:
  - run: picker
...
`;
    ...
    const yamlFile = join(tmpDir, 'smoke-cdx-wf.yaml');
